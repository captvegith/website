...
// Choix Offres => prix plantesU
    ...
    function numberOfPlantes(){
        var qtyOfPlantes = 0; 
        var theForm = document.forms["pricingPlantes"];
        var plantesQuantity = theForm.elements["quantity"];

        for (var i = 0; i < plantesQuantity.length; i++) {

                qtyOfPlantes += parseInt(plantesQuantity[i].value);
        }
        return qtyOfPlantes; 
    }; 
    document.getElementById('totalPlanteQty').value = numberOfPlantes(); 

    // Cout entretien par mois 
    function costEntretien() {
        ...
        if(document.getElementById('offerAchatEntretien').checked || (document.getElementById('offerLocation').checked)) {
            coutEntretien = nombreOfPlantes + 60; 
        }; 
        return coutEntretien; 
    }; 
    document.getElementById('coutEntretienT').value = costEntretien(); 
...
// fonction calcul de la quantité totale de plantes 
    // // fonction cout plantes avec pots : 
    // function plantesPlusPots(){
    //     var 
    // }

    function costLocation() {
